...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    if (!body.confirmSensitive) {
      return fail("Confirme a ação sensível para gerar o SQL com credenciais.");
    }
    ...
    const sqlContent = buildManualSql(existing.databaseUser, password, existing.databaseName);

    await writeAudit(prisma, {
      action: "PROJECT_DATABASE_SQL_GENERATED",
      entityType: "project_database",
      entityId: existing.id,
      entityName: existing.name,
      userId: user.id,
      description: `SQL manual gerado para o banco ${existing.name}.`,
      newData: sanitizeProjectDatabaseForAudit(toProjectDatabase(existing))
    });

    return ok({
      sqlContent,
      warning: "Revise antes de executar em produção."
    });
  } catch (error) {
