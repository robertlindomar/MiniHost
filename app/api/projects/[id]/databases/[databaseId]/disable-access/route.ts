...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const updated = await disableProjectDatabaseAccess(existing.id, user.id);

    await writeAudit(prisma, {
      action: "PROJECT_DATABASE_DISABLE_ACCESS",
      entityType: "project_database",
      entityId: updated.id,
      entityName: updated.name,
      userId: user.id,
      description: `Acesso desativado para o usuário ${updated.databaseUser} no banco ${updated.databaseName}.`,
    ...
    return ok({
      message: "Acesso ao banco desativado com sucesso.",
