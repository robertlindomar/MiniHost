...
import {
  defaultDatabaseListFilter,
  isDatabaseDestroyed,
  matchesDatabaseListFilter,
  type ProjectDatabaseListFilter
...
const FILTER_OPTIONS: { value: ProjectDatabaseListFilter; label: string }[] = [
  { value: "default", label: "Padrão (sem destruídos)" },
  { value: "active", label: "Ativos" },
  { value: "disabled", label: "Desativados" },
  { value: "archived", label: "Arquivados" },
  { value: "destroyed", label: "Destruídos" },
...
export function ProjectDatabaseTable({
  ...
  const [filter, setFilter] = useState<ProjectDatabaseListFilter>(defaultDatabaseListFilter());

  const filteredDatabases = useMemo(
    () => databases.filter((database) => matchesDatabaseListFilter(database.status, filter)),
    [databases, filter]
  );
    ...
    <div className="space-y-4 p-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-sm text-zinc-600">
          {filteredDatabases.length} de {databases.length} banco(s) visíveis
        </p>
        <label className="flex items-center gap-2 text-sm text-zinc-700">
          <span className="font-medium">Filtrar:</span>
          <select
            value={filter}
            onChange={(event) => setFilter(event.target.value as ProjectDatabaseListFilter)}
      ...
      {filteredDatabases.length === 0 ? (
        <div className="rounded-lg border border-dashed border-zinc-200 bg-zinc-50 px-4 py-8 text-center text-sm text-zinc-600">
          Nenhum banco corresponde ao filtro selecionado.
        </div>
      ) : (
          ...
          <table className="min-w-[1180px] w-full border-collapse">
            ...
            <tbody className="divide-y divide-zinc-100">
              {filteredDatabases.map((database) => {
                ...
                const canEdit = database.status !== "ARCHIVED" && !destroyed && !isArchivedProject;
