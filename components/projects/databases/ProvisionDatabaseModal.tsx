...
import { buildMaskedProvisionSql, buildProvisionConfirmationText } from "@/lib/provision";
import type { Project, ProjectDatabase } from "@/lib/types";

interface ProvisionDatabaseModalProps {
...
export function ProvisionDatabaseModal({
  database,
  project,
  isOpen,
  isSubmitting = false,
  ...
  const expectedText = buildProvisionConfirmationText(database.databaseName);
  const maskedSql = buildMaskedProvisionSql(database.databaseUser, database.databaseName);
  const isValid = confirmationText === expectedText;

  return (
    <Modal isOpen={isOpen} title="Criar banco real" onClose={onClose}>
      <div className="space-y-5">
        <p className="text-sm leading-6 text-zinc-600">
          Esta ação criará o usuário e o banco PostgreSQL no servidor usando a credencial administrativa
          configurada em Configurações. Revise os dados antes de confirmar.
        </p>
        ...
        <CodeBlock content={maskedSql} label="SQL que será executado (senha mascarada)" />

        <div>
          <label htmlFor="provision-confirmation" className="block text-sm font-medium text-zinc-700">
            Confirmação
          </label>
